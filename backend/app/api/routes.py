...
# Import from your new structured folders
...
async def analyze_batch(job_description: str = Form(...), files: List[UploadFile] = File(...)):
    ...
    # --- STEP 2: CLEAR OLD STATE (Safe to destroy now) ---
    ...
    await asyncio.sleep(3)  # Simulate processing time for better UX with spinner
    await ai_engine.initialize_rag_vectorstore(TEMP_DIR)

    # Clean up PDFs after vectorization to save disk space
    for path in saved_paths:
